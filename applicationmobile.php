...
<script>
    const text = "Avec A³I, développez des applications mobile personnalisées adaptées aux besoins spécifiques de votre entreprise, avec des interfaces intuitives et des fonctionnalités avancées..";
...
<style>
  ...
  <form id="contactForm" action="send_email.php" method="POST" class="row g-3">
    <!-- Formulaire (comme précédemment) -->

    <div class="col-md-6">
        <label for="nameWeb" class="form-label">Nom complet</label>
        <input type="text" class="form-control" id="nameWeb" name="name" placeholder="Entrez votre nom" required>
    </div>

    <div class="col-md-6">
        <label for="emailWeb" class="form-label">Email</label>
        <input type="email" class="form-control" id="emailWeb" name="email" placeholder="Entrez votre email" required>
    </div>
    
    <div class="col-md-6">
        <label for="phoneWeb" class="form-label">Numéro de téléphone</label>
        <input type="tel" class="form-control" id="phoneWeb" name="phone" placeholder="Ex: 98 *** ***" pattern="^[0-9]{8}$" required>
    ...
    <div class="col-md-6">
        ...
        <select class="form-select" id="appTypeWeb" name="appType" required>
    ...
    <div class="col-md-6">
        ...
        <select class="form-select" id="themeWeb" name="theme" required>
    ...
    <div class="col-12">
        ...
        <textarea class="form-control" id="descriptionWeb" name="description" rows="5" placeholder="Décrivez votre projet et ses fonctionnalités principales" required></textarea>
    </div>

    <div class="col-12">
        <label for="featuresWeb" class="form-label">Fonctionnalités supplémentaires</label>
        <div class="form-check">
            <input class="form-check-input" type="checkbox" name="features[]" value="responsive" id="responsiveWeb">
            <label class="form-check-label" for="responsiveWeb">Design responsive</label>
        </div>
        <div class="form-check">
            <input class="form-check-input" type="checkbox" name="features[]" value="cms" id="cmsWeb">
            <label class="form-check-label" for="cmsWeb">Système de gestion de contenu (CMS)</label>
        </div>
        <div class="form-check">
            <input class="form-check-input" type="checkbox" name="features[]" value="seo" id="seoWeb">
            <label class="form-check-label" for="seoWeb">Optimisation pour les moteurs de recherche (SEO)</label>
        </div>
        <div class="form-check">
            <input class="form-check-input" type="checkbox" name="features[]" value="analytics" id="analyticsWeb">
    ...
    <div class="col-md-6">
        ...
        <input type="number" class="form-control" id="budgetWeb" name="budget" placeholder="Exemple : 5000 TND" required>
    </div>

    <div class="col-md-6">
        <label for="deliveryDateWeb" class="form-label">Date de livraison souhaitée</label>
        <input type="date" class="form-control" id="deliveryDateWeb" name="deliveryDate" required>
    </div>
...
<script>
    $(document).ready(function () {
        $('#contactForm').on('submit', function (event) {
            ...
            $.ajax({
                url: 'send_email.php', // URL du fichier PHP
                type: 'POST',
                ...
                error: function (jqXHR, textStatus, errorThrown) {
                    alert("Erreur lors de l'envoi : " + textStatus);
                }
            });
        });
    });
